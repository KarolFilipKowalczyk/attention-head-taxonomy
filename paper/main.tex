...
This taxonomy uses \emph{stacks} as organizational framework. Attention heads work in complex \emph{circuits}---groups of components across layers cooperating through multi-level processing~\cite{elhage2021mathematical,wang2022interpretability}. Circuits involve not just attention heads but also MLPs, layer norms, and residual connections. A single computational task (e.g., indirect object identification) requires coordinated activation across 10+ heads and multiple MLP layers, with information flowing through intricate paths in the residual stream.

The \emph{stack} abstraction simplifies this complexity for communication. Rather than mapping every circuit connection or MLP contribution, I group heads by primary functional contribution, making the taxonomy accessible while acknowledging that real model behavior involves intricate cross-layer, cross-component interactions. Attention heads provide tractable, interpretable entry points into these larger computational structures.
...
\subsection{Polyfunctional Heads and Naming Scope}
Attention heads commonly activate multiple behaviors and participate in multiple circuits. Naming each head by its dominant function provides stable vocabulary while acknowledging that heads contribute to secondary stacks. A refusal head may influence tone; an induction head may support entity tracking. This taxonomy labels the primary, reproducible function rather than cataloging all possible activations.
...
\subsection{Beyond Attention: MLPs and Circuits}
Transformer computation involves more than attention heads. MLPs store factual knowledge, perform nonlinear transformations, and route information~\cite{elhage2021mathematical}. Layer norms and residual connections enable information flow across depths. Circuits combine these components: the IOI circuit uses duplicate-token heads, S-inhibition heads, name-mover heads, and supporting MLPs across 6--8 layers~\cite{wang2022interpretability}. This taxonomy focuses on attention heads as interpretable, well-studied components, not because they work alone but because naming them aids circuit understanding.
...
A \emph{stack} groups head types that together implement a higher-level capability. Stacks reflect functional clustering observed in studies~\cite{wang2022interpretability,olsson2022context}. Examples: Reasoning \& Algorithmic, Memory \& Dependency, Safety, Output Formatting. Stacks span Early, Middle, Late, and Final layers. Stack membership reflects each head's primary contribution, though many heads play secondary roles in other stacks.

Stacks simplify circuit complexity. The IOI circuit demonstrates this: duplicate-token heads (M) detect repeated names, S-inhibition heads (L) suppress incorrect entities, name-mover heads (L) copy correct names to output---all coordinated with MLPs that modulate attention patterns and refine representations. This taxonomy names the attention components; the full circuit includes MLP contributions at each stage.
...
Polysemanticity is the norm rather than the exception. Most heads participate in multiple circuits, activate different behaviors across contexts, and contribute to multiple computational goals. This naming convention standardizes the dominant mode: the function most consistently activated, most central to the head's circuit role, and most reproducible across architectures~\cite{voita2019analyzing}. Secondary behaviors appear in descriptions and cross-references. Single names cannot capture full functionality, but they provide stable communication anchors for the primary, empirically dominant function.

\subsection{Circuit Complexity and Component Interactions}
Naming attention heads does not imply they operate independently. Circuits involve coordinated computation across heads, MLPs, and layers~\cite{elhage2021mathematical,wang2022interpretability}. Induction requires previous-token heads (E) feeding into induction heads (M). IOI requires duplicate-token heads (M), S-inhibition heads (L), name-mover heads (L), and MLP layers that sharpen attention patterns. Safety enforcement combines content-detection heads (E), policy-enforcement heads (L), refusal heads (F), and MLPs that modulate intervention strength. 

Head names identify components within these circuits, not isolated functions. A ``name-mover head'' moves names \emph{in the context of} supporting circuits. Understanding requires mapping full computational paths: which heads attend where, which MLPs process which representations, how information flows through residual streams across 20--100 layers. This taxonomy provides vocabulary for the attention components of these larger structures.
...
\paragraph{Scope.} This taxonomy focuses exclusively on attention heads, the most interpretable and well-studied transformer components. MLPs contribute critically to model computation: storing factual associations, performing nonlinear transformations, modulating attention outputs, and routing information between layers~\cite{elhage2021mathematical}. Embeddings encode initial token representations. Layer norms stabilize activations. Residual connections enable information flow. A complete understanding requires all components, but attention heads provide tractable entry points. Future taxonomies should extend to MLPs and their functional types.
