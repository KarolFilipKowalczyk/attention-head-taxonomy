...
\begin{ablationbox}
\textbf{Expected ablation:} Increased format inconsistency and reduced output polish (~20-35\% more style variations and ~15-30\% quality degradation). Mixed indentation, inconsistent capitalization, varying syntax choices. More awkward phrasings, occasional grammatical rough spots, less fluent prose. Output remains functional but less polished and professional-appearing. Functional but less refined outputs. Particularly noticeable in long structured outputs. Partial compensation through earlier generation quality.
\end{ablationbox}
